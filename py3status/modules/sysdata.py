...
class Py3status:
    ...
    def _get_meminfo(self, head=49):
        with open("/proc/meminfo") as f:
            fields = [line.split() for line in (next(f) for x in range(head))]
            return {field[0]: float(field[1]) for field in fields}
    ...
    def sysdata(self):
        ...
        if self.init["meminfo"]:
            meminfo = self._get_meminfo(head=24)
